...
React hooks are functions that let us "hook" into React state and lifecycle features from function components. Hooks allow us to to manipulate the state of functional components without needing to convert into class components. We also can use them to achieve similar effects as common lifecycle methods in function components.
